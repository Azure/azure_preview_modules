...
version_added: "2.5"
short_description: Manage Webhooks instance.
description:
    - Create, update and delete instance of Webhooks.
...
EXAMPLES = '''
  - name: Create (or update) Webhooks
...
class AzureRMWebhooks(AzureRMModuleBase):
    """Configuration class for an Azure RM Webhooks resource"""
    ...
    def exec_module(self, **kwargs):
        ...
        if not old_response:
            self.log("Webhooks instance doesn't exist")
            if self.state == 'absent':
                self.log("Old instance didn't exist")
            else:
                self.to_do = Actions.Create
        else:
            self.log("Webhooks instance already exists")
            if self.state == 'absent':
                self.to_do = Actions.Delete
            elif self.state == 'present':
                self.log("Need to check if Webhooks instance has to be deleted or may be updated")
                self.to_do = Actions.Update

        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            self.log("Need to Create / Update the Webhooks instance")

            if self.check_mode:
                self.results['changed'] = True
                return self.results

            response = self.create_update_webhooks()
            ...
            self.log("Webhooks instance deleted")
            self.results['changed'] = True
            ...
            while self.get_webhooks():
                time.sleep(20)
        else:
            self.log("Webhooks instance unchanged")
    ...
    def create_update_webhooks(self):
        '''
        Creates or updates Webhooks with the specified configuration.

        :return: deserialized Webhooks instance state dictionary
        '''
        self.log("Creating / Updating the Webhooks instance {0}".format(self.webhook_name))
        ...
        try:
            ...
            self.log('Error attempting to create the Webhooks instance.')
            self.fail("Error creating the Webhooks instance: {0}".format(str(exc)))
        return response.as_dict()

    def delete_webhooks(self):
        '''
        Deletes specified Webhooks instance in the specified subscription and resource group.

        :return: True
        '''
        self.log("Deleting the Webhooks instance {0}".format(self.webhook_name))
        try:
            response = self.mgmt_client.webhooks.delete(resource_group_name=self.resource_group,
                                                        registry_name=self.registry_name,
                                                        webhook_name=self.webhook_name)
        except CloudError as e:
            self.log('Error attempting to delete the Webhooks instance.')
            self.fail("Error deleting the Webhooks instance: {0}".format(str(e)))

        return True

    def get_webhooks(self):
        '''
        Gets the properties of the specified Webhooks.

        :return: deserialized Webhooks instance state dictionary
        '''
        self.log("Checking if the Webhooks instance {0} is present".format(self.webhook_name))
        found = False
        try:
            ...
            self.log("Webhooks instance : {0} found".format(response.name))
        except CloudError as e:
            self.log('Did not find the Webhooks instance.')
        if found is True:
            return response.as_dict()
