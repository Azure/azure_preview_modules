...
class AzureRMStorageAccount(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        for key in self.storage_accounts_models.SkuName:
    ...
    def update_account(self):
        ...
        if self.account_type:
            if self.account_type != self.account_dict['sku_name']:
                ...
                SkuName = self.storage_accounts_models.SkuName
                ...
                if self.results['changed'] and not self.check_mode:
                    ...
                    try:
                        ...
                        sku = self.storage_accounts_models.Sku(SkuName(self.account_dict['sku_name']))
                        sku.tier = self.storage_accounts_models.SkuTier(self.account_dict['sku_tier'])
                        parameters = self.storage_accounts_models.StorageAccountUpdateParameters(sku=sku)
        ...
        if self.custom_domain:
            ...
            if self.results['changed'] and not self.check_mode:
                new_domain = self.storage_accounts_models.CustomDomain(name=self.custom_domain['name'],
                                          use_sub_domain=self.custom_domain['use_sub_domain'])
                parameters = self.storage_accounts_models.StorageAccountUpdateParameters(custom_domain=new_domain)
                try:
        ...
        if self.access_tier:
            ...
            if self.results['changed'] and not self.check_mode:
                parameters = self.storage_accounts_models.StorageAccountUpdateParameters(access_tier=self.access_tier)
                try:
                    self.storage_client.storage_accounts.update(self.resource_group, self.name, parameters)
                except Exception as exc:
        ...
        if update_tags:
            ...
            if not self.check_mode:
                parameters = self.storage_accounts_models.StorageAccountUpdateParameters(tags=self.account_dict['tags'])
                try:
                    self.storage_client.storage_accounts.update(self.resource_group, self.name, parameters)
                except Exception as exc:
    ...
    def create_account(self):
        ...
        if self.check_mode:
            ...
            return account_dict
        sku = self.storage_accounts_models.Sku(self.storage_accounts_models.SkuName(self.account_type))
        sku.tier = self.storage_accounts_models.SkuTier.standard if 'Standard' in self.account_type else \
            self.storage_accounts_models.SkuTier.premium
        parameters = self.storage_accounts_models.StorageAccountCreateParameters(sku, self.kind, self.location,
    ...
    def delete_account(self):
        if self.account_dict['provisioning_state'] == self.storage_accounts_models.ProvisioningState.succeeded.value and \
